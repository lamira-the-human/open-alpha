      ...
      <main className="container" style={{ padding: '1.5rem 1rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginLeft: 'auto' }}>
            {(['tiles', 'graph'] as const).map(mode => (
              <button
                key={mode}
                onClick={() => setViewMode(mode)}
                className={viewMode === mode ? 'btn btn-primary' : 'btn btn-outline'}
                style={{ padding: '0.4rem 0.875rem', fontSize: '0.875rem', textTransform: 'capitalize' }}
              >
                {mode === 'tiles' ? '⊞ Tiles' : '⬡ Graph'}
              </button>
            ))}
            <button
        ...
        {viewMode === 'tiles' ? (
          /* Tile grid */
          <div style={{ border: '1px solid var(--border)', borderRadius: '0.75rem', background: 'var(--surface)', padding: '1.25rem' }}>
            {byLevel.map((levelConcepts, level) => {
              if (levelConcepts.length === 0) return null;
              return (
                <div key={level} style={{ marginBottom: '1.25rem' }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-light)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.5rem' }}>
                    {level === 0 ? 'Kindergarten' : `Grade ${level}`}
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                    {levelConcepts.map(concept => {
                      ...
                      return (
                        <div
                          key={concept.id}
                          onClick={() => clickable ? navigate(`/learn/${subject}/${concept.id}`) : setTooltip(tooltip?.id === concept.id ? null : { id: concept.id, x: 0, y: 0 })}
                          title={status === 'locked' ? `Requires: ${concept.prerequisites.map(pid => concepts.find(c => c.id === pid)?.name ?? pid).join(', ')}` : concept.name}
                          style={{
                        ...
                        >
                          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {status === 'mastered' ? '✓ ' : status === 'locked' ? '🔒 ' : ''}{concept.name}
                          </span>
                          {concept.masteryScore > 0 && (
                            <span style={{ fontSize: '0.7rem', opacity: 0.8 }}>{concept.masteryScore}%</span>
                          )}
                        </div>
                      );
                    })}
